concept :the_false_form do
  ...
  prose <<~PROSE, section: :perception, heading: "What the Bench Knows"
    People who work where figures come apart have a quieter relationship with the idea. #{ref :tuners, "Tuners"} who run benches near the #{ref :bloom_zones, "Bloom Zones"}, salvagers who handle raw crystal out of the #{ref :deep_shear, "deep Shear"}, and anyone who has spent too long around #{ref :duskgrain, "duskgrain"} use the word *rhomboid* less lightly than city folk. Their instruments offer no common theory, but their work repeatedly finds places and substances where a vibration will not close, where a tuned figure that holds elsewhere refuses to hold, and where prolonged exposure makes a person's own edges feel uncertain in a way the #{ref :fae, "fae"} recognize.
  ...
  prose <<~PROSE, section: :truth, heading: "The Hidden Truth", dm: true
    *Falsehood is not*, the line on which the system's public teaching rests, is the most useful lie the #{ref :elves, "elves"} ever let stand. The False Form is the *Adversary*, a real and active geometry with will.
  PROSE
  prose <<~PROSE, section: :truth, heading: "The Cosmology Is Literally True", dm: true
    The #{ref :the_three_forms, "Three Forms"} accurately describes how resonance and reality work. Its makers knew what the third panel represented and chose to draw it as a devil rather than a coordinate.
  ...
  prose <<~PROSE, section: :truth, heading: "It Has Will", dm: true
    The False Form has agency. Entropy is indifferent; the Adversary chooses. It pressed *here* rather than elsewhere, timed the #{ref :the_glassfall, "Glassfall"}, and, according to #{ref :elves, "the elves"}, will return. The "one conflict" written under the triptych describes a war against an active opponent.

    Elven records do not establish the nature of that agency. The Adversary may be a single intelligence, a swarm, a law of another cosmos that experiences this world's refusal to dissolve as an intrusion, or something for which "intelligence" is the wrong frame. The records establish only that it comes from outside the Kaleidos system, can destroy ring-scale megastructures, acts with apparent intent, and is expected by the #{ref :elves, "elves"} to return. Their preparations record the coming war; #{ref :duskgrain, "duskgrain"} records what its presence leaves behind.
  PROSE
...
relate :rel_the_false_form_caused_duskgrain, :caused, :the_false_form, :duskgrain, dm: true do
  prose "#{ref :duskgrain, "Duskgrain"} is the False Form made matter: crystallized fragments of the figure that will not close. Prolonged exposure tunes a person toward its alien geometry."
end
