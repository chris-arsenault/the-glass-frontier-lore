encyclopedia :marrow_kite do
  ...
  prose <<~PROSE
    ...
    Kept kites are the Famine's gift to the signal trades. When the relays went dark, hab keepers manned wild-taken nestlings — the falconer's word survives in the trade — and flew them between stations with message capsules on their legs, and the practice outlived the emergency because a kite crosses a gap in band weather that grounds flitters and scrambles relays. A manned bird knows its keeper by voice and gait, flies to task for the lure and the crop, and comes home to the wrist. Keepers weigh their birds every morning with a devotion outsiders mistake for ritual; it is arithmetic. A kite at flying weight works, a kite above it sits on a strut and reviews its options, and the scales settle the question before the gap does.
  PROSE

  prose <<~PROSE, section: :operations, heading: "The Keeper's Round"
    A working kite's day is a keeper's ledger: the weighing, the crop ration measured against the day's task, the anklets checked, the bell — each keeper's bell tuned distinct — wrapped or freed depending on whether the flight wants announcing. Message work runs station to station along trained lines, capsule on the leg, and survey work runs circuits over claims and cordons with the bird's turns logged from below. The trade's quiet pride is the storm launch: a kite put up into weather that has silenced every relay in the district, carrying the words that matter most exactly when everything else has stopped.
