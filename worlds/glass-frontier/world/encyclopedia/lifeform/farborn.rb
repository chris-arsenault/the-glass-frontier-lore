encyclopedia :farborn do
  ...
  prose <<~PROSE
    ...
    Bestiaries divide them into convenient working categories — #{encyclopedia_ref :drift_eel, "drift-eels"}, #{encyclopedia_ref :shellback, "shellbacks"}, and the rest — but the categories are habits and habitats, not lineages. What unifies the beasts is the place they inhabit, the impossible features they share, and the open question of where they came from.
  PROSE
  ...
  affordance "Players ask what the thing in front of them is. A working crew answers with a behavior name — deckmouth, flintjack, clatter, glowbag — because the field term decides which shutter closes and which tone goes dark, and the name gets revised later if the specimen can be examined."
  pressure "Put a Tuner in resonance contact with one and they report it is not quite where it is, with no instrument recording the discrepancy. Researchers who have not worked around the beasts dismiss this; containment Tuners say it matches what they feel near a Bloom Zone at lower amplitude."
