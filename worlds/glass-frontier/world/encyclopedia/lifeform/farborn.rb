encyclopedia :farborn do
  ...
  prose <<~PROSE
    *Farborn* is the collective name for unrelated creatures found in #{ref :the_shear, "the Shear"}. Bodies range across sizes, materials, and forms that support no accepted common lineage. Working bestiaries divide them by habitat and behavior into categories such as #{encyclopedia_ref :drift_eel, "drift-eels"} and #{encyclopedia_ref :shellback, "shellbacks"}.
  ...
  prose <<~PROSE, section: :operations, heading: "Cutter-Wake Names"
    Crews using #{encyclopedia_ref :remote_cutting, "remote cutters"} need names that tell them what an intruder will do in the receiving bay. #{encyclopedia_ref :ratters, "Ratter"} boards therefore record cutter-wake beasts by behavior. The terms overlap with the broader bestiary and make no claim about lineage.
  ...
  prose <<~PROSE, section: :mechanics, heading: "Where They Come From"
    ...
    The #{ref :echo_ledger_conclave, "Echo Ledger Conclave"} and the #{ref :clarisant, "Clarisant"} Academy publish morphology and behavior while withholding an origin judgment. Recovered bodies degrade, deep-runner testimony is difficult to repeat under controlled conditions, and no instrument has observed a crossing from its far side. Researchers with substantial Deep field time more often accept the crew account; the institutional position requires a stable specimen or repeatable observation of passage.
  PROSE
  prose <<~PROSE, section: :operations, heading: "Hunting and Bounty"
    ...
    Named bounties draw several hunters or flotillas across the jurisdictions the beast crosses. A confirmed kill enters the hunter beside the animal in Compact records and in the songs that follow. **Geld Marrick** brought down **Pale Star** in 2376 and is still introduced through that hunt; later songs enlarge its details beyond the surviving contract and vessel logs.
  ...
  cue "When a named beast takes a ship, the affected hab posts a bounty and the professional hunters converge. The hunter who brings one down is named beside it permanently, the way Geld Marrick has been introduced by Pale Star since 2376."
  ...
  pressure "Tuners in resonance contact with a Farborn report its body displaced slightly from the point instruments assign it. Containment Tuners compare the sensation to a Bloom Zone at lower amplitude; published instruments have not recorded the discrepancy."
