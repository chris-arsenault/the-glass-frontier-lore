encyclopedia :claim_rig do
  ...
  prose <<~PROSE
    A claim rig is a salvage vessel built to receive material from unstable debris without bringing its crew alongside the claim. It carries one or more unmanned cutters, a separate receiver for each cutter, and enough processing rooms to decide what has come back before opening it to the rest of the ship. #{encyclopedia_ref :ratters, "Ratters"} call almost every vessel they fly a skiff. A claim rig is the kind of skiff whose interior is arranged around doors that may need to close quickly.
