encyclopedia :heavy_hauler do
  ...
  prose <<~PROSE, section: :operations, heading: "Coupled Freight"
    Cargo from #{encyclopedia_ref :remote_cutting, "Claim Coupling"} receives a separate loading plan. Material recovered on different cutter clocks cannot share a resonance bus, a structural brace, or a water tank. The bays travel dark, with their monitoring instruments isolated from the vessel's main cadence.
  ...
  prose <<~PROSE, section: :mechanics, heading: "Construction"
    A bulk kite is built around its #{encyclopedia_ref :kite_sail, "kite"} arrays and its cargo capacity, in that order. Everything else — crew quarters, sensors, hull configuration — is laid out around the two structural priorities.
  ...
  prose <<~PROSE, section: :people, heading: "Crew"
    ...
    Keel runs require a Clarisant-credentialed #{encyclopedia_ref :tuners, "Tuner"} under Shear Compact rules. The Tuner monitors array health and manages structural reinforcement through disturbed fields. Runs with armed escorts also assign one officer to the formation's shared traffic and maneuvering channel.
  PROSE
