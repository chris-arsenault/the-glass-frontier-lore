encyclopedia :tuning_compass do
  ...
  prose <<~PROSE, section: :history, heading: "From Local Tools to a Shared Reference"
    ...
    After the Rekindling, instrument makers compared those local tools against the emerging band model. The present three-reed form spread because it could be repaired with ordinary materials and checked against any well-kept #{encyclopedia_ref :resonant_instruments, "resonant instrument"}. The #{ref :clarisant, "Clarisant"} publishes reference tones and tests the compasses used in its own work, while independent makers retain different case shapes, reed alloys, and local markings.
  PROSE
