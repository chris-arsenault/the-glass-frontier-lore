encyclopedia :cordon_station do
  name "Bloomwatch"
  kind :technology
  subkind :border_post
  status :complete
  ...
  prose <<~PROSE
    Cordon services build the same low, damped bloomwatch along every established Bloom boundary. The standard plan has three working spaces. On the watch floor, crews record band instruments, survey-stake bearings, and reports from #{encyclopedia_ref :bloom_sense, "sensitives"}. The gear room holds stakes, signal bells, closure tackle, stretchers, and the sensitive's chair. Quarters keep a crew close enough to walk the line at dawn and dusk and replace any markers moved overnight. Every record is copied, and one set travels outward on each supply run.
