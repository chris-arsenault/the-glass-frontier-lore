encyclopedia :drawdown do
  title "Drawdown"
  kind :phenomenon
  subkind :physical_condition
  status :complete
  ...
  cue "Broad-band devices give way first: water heats slowly, a lantern dims, and a signal set sheds distance."
  cue "A tuning compass swings toward the strongest remaining gradient and then stops moving as the field evens out."
  affordance "Settlements schedule energy-intensive work for low-demand periods and store heat, pressure, and charge in ordinary physical forms."
  pressure "A kite keeps its momentum when the kinetic band thins, and a reinforced span keeps carrying traffic when its structural support weakens."
  variation "A broad application may keep working while a focused device fails because its usable frequency has gone quiet."
  variation "Cracks, dry gaps, and differently tuned sections slow or divert a draw through otherwise connected material."
  ...
  prose <<~PROSE, section: :dangers, heading: "Failure Signs"
    Drawdown is usually gradual enough to catch. A #{encyclopedia_ref :tuning_compass, "tuning compass"} turns toward the strongest remaining gradient, then loses motion as the field evens out. Attuned workers feel one band fade or notice that a familiar response arrives late. Well-maintained equipment drops output or shuts itself down.
