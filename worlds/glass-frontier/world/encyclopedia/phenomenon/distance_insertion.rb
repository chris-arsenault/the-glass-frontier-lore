encyclopedia :distance_insertion do
  ...
  prose <<~PROSE
    ...
    Survey models compare middles with the shortened routes traced by #{encyclopedia_ref :fold_spider, "shearweavers"}: both produce repeatable differences between local travel and exterior measurement, with opposite signs. Current models describe the geometry but do not account for fixed endpoints together with changing internal depth. Registers therefore treat depth and drift as measured conditions, independent of the proposed cause.
  PROSE
