encyclopedia :kesh_challenge_names do
  ...
  descriptive_identity(
    manner: "Kesh introductions lead with the challenge name and offer the household name " \
            "second, if at all — 'Crossed the Weir Deck' shakes your hand before anyone " \
            "named Tam does — and complimenting a name is done by asking, technically, " \
            "how it was earned.",
  ...
  prose <<~PROSE
    ...
    The trial board keeps the whole apparatus honest. Challenges post with their conditions — route, load, spin state, the assistance line that must stay untouched — completions require two witnesses off the challenger's crew, and the board's register pairs every current name in Kesh with its date and witnesses. Retired names pass into the recitation rolls, and the board's grading sessions, where new trials are argued into the canon and old ones re-graded as the settlement's spin drifts, are Kesh's best-attended public entertainment. The culture's sharp edge is generational: the spin state that made a grandmother's name legendary may be gone with the junction's drift, and the board's re-grading of the old trials against the current physics is the argument every Kesh family is having at every table, permanently, on principle.
  PROSE
  ...
  pressure "The name is only as current as the body that earned it, and Kesh's drifting spin re-grades old feats under everyone — so the board's re-grading sessions decide whose legend survives the physics, in public, with the family listening."
