encyclopedia :breath_debt do
  ...
  prose <<~PROSE
    Air in a sealed hab is the one stock that admits no substitutes, and the ring settlements long ago worked out how to owe it. Breath debt is the accounting: when a district's #{encyclopedia_ref :bell_lung_colossus, "clock"} fails or its shafts close for repair, it draws on its neighbors' stored air — valves opened between districts, the draw metered at the boundary — and the measured volume enters the ledgers as debt of the specific kind. Air debt is repaid in air. The owing district returns the volume at the lender's need, whenever that need arrives, and the ledgers run for generations because the need runs on its own schedule: a draw taken in a grandmother's repair season may be returned in a grandchild's closure, at the same boundary valve, entered against the same line.
