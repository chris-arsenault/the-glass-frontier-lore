encyclopedia :gray_table do
  ...
  prose <<~PROSE
    ...
    The custom descends from the Famine's first winters, when the setting of places was less rite than refusal — households that had eaten beside elven neighbors for generations declining, at the table, to accept the disappearance as final — and it has kept that refusal's grain ever since. Table-keepers are a small minority and know each other; the keepings pass down lines with the ledgers and crocks that name the guests; and the custom's relation to doctrine is deliberately unresolved. The Communion houses have never claimed it, the keepers rarely explain it, and the one liturgical sentence most tables share is the toast that closes the meal, old enough that its dialect varies: the places are kept, the work is kept, and the door is unbarred.
  PROSE
  ...
  pressure "The custom is a standing refusal to close the system's largest open account, and it sits differently on each generation that inherits it — the keeping that comforts the grandmother can weigh on the grandchild, and lapsing a table is a decision families circle for years."
