encyclopedia :calling_in do
  ...
  prose <<~PROSE
    ...
    Habs teach the call and the answer together, young, with the weight of a survival skill, because the pairing is one. The answer is mandatory — a hab child learns that an unanswered call starts a search, and that this is precisely the system working — and the roll-call customs built on the pairing run through everything: households calling in at the evening bell, work crews calling the roster down a dark section, the dock offices' closure drill that calls every name on the manifest through the sealed district and marks the answers on the board. The three call tones are the craft's entire grammar — ordinary, urgent, and the flat emergency call that empties bunks — held to three by a discipline every settlement enforces, on the shared understanding that the call's power is exactly its poverty: a name, a tone, and an answer owed.
  PROSE
  ...
  pressure "The unanswered call starts a search by design, so the custom binds both ways — the person out of earshot owes the hab a told absence, and the caller who cries a name idly has spent a coin the whole system runs on."
