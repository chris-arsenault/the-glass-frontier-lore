encyclopedia :ledger_recall do
  ...
  prose <<~PROSE
    ...
    The trade's standing is juridical. A certified recaller taking an account is a recording in law — courts, registries, and freight houses accept witnessed replay as they accept a sealed page — and the certification examines exactly what it must: verbatim fidelity across seasons, replay under distraction, and the discipline's characteristic honesty about its own state. Fidelity degrades with injury, exhaustion, and grief, the training makes the degradation feel like what it is, and the recaller's oath binds them to declare it; 'my wrist is tired' is a formal statement, adjourns proceedings without prejudice, and is treated across the system with the gravity of an instrument reporting its own drift.
  PROSE

  prose <<~PROSE, section: :structure, heading: "Holders, Hearings, and the Third Wrist"
    Working recallers cluster where records must move or multiply. Carrier lines keep one on the long routes, holding manifests and the season's contracts between archives. Courts keep a bench recaller for testimony taken where no clerk can go. The registries' habit is the third wrist: an account of consequence is taken by two recallers at once and archived on paper besides, and discrepancies between the three — rare, logged, and studied — are the discipline's own quality literature.

    Training runs young, since the binding takes best in a wrist still learning its patterns. Recaller families and clerical schools start children on nursery accounts — a rhyme bound and replayed across a winter — and the profession's examination piece is traditional everywhere: the candidate takes a full market-day's disputes at the bench, sleeps, and replays the lot to the magistrates in order, with the original hesitations, while the disputants listen to themselves think again.
  PROSE
