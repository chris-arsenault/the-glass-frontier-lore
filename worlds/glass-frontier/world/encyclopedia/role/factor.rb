encyclopedia :factor do
  ...
  pressure "The book prices character as well as cargo, so a factor's quiet estimate moves real fortunes — and the trade's discretion rules exist because everyone knows exactly how much a raised eyebrow at the fair-day table costs."
  ...
  prose <<~PROSE
    Trade outruns coin wherever seasons, distances, and disasters intervene, and the system's answer sits at the fair-day table. A factor keeps accounts between parties who cannot settle now — the crew paid in shares of a cargo two months out, the farm advanced against a harvest, the road-debt owed from last season's rescue on the far side of the system — and the factor's book makes the obligations liquid: entries in a certified hand, cleared with the counter-seal, honored between corresponding factors along every carrier line. The trade rose from the Famine's barter ledgers, when the keepers of the community books became, by demonstrated honesty, the people through whom a cut-off settlement's promises could still circulate, and the profession keeps its founding discipline as furniture — the locked satchel, the murmured transaction, the scales on the counter polished and mostly idle.
  ...
  prose <<~PROSE, section: :structure, heading: "Clearing, Correspondence, and the Fair"
    ...
    Correspondence binds the local books into a system. Factors keep certified relationships along the routes — hands examined, seals exchanged, liabilities bounded in the dry contractual prose the trade savors — so a debt of the road contracted at a Dovra ford clears at a Keelward counter a season later on a slip and a seal. The network's discipline is its own enforcement: a factor whose marks come back disputed answers to every correspondent at once, and the trade's histories dwell on the handful of famous failures with the loving detail other professions give their triumphs.
  PROSE
end
