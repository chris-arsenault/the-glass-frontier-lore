encyclopedia :mender do
  name "Mender"
  summary "A mender is an itinerant repair tradesman working a posted circuit with a treadle cart — edges ground, vessels patched, seals renewed, small mechanisms coaxed — arriving on the schedule the circuit keeps and announced by the cart's chime bar. One cart supplies workshop tools to every small settlement on its route."
  kind :role
  subkind :profession
  status :complete
  ...
  pressure "The circuit posts its schedule and names each price before work begins. Settlements plan repairs for the cart's arrival, so the mender records delays at the road registry and sends notice ahead when a season's round slips."
  ...
  prose <<~PROSE
    #{encyclopedia_ref :elves, "Elven"} districts kept fabricators as public infrastructure, with ordinary repair available in every neighborhood. After the Glassfall, a fragment village of forty households might keep a forge, a seal press, or a hand skilled at small mechanisms, but seldom all three. The mender carries the missing work between settlements. Cart, treadle wheel, forge cup, seal press, and a drawer bank of salvaged fittings make a workshop on axles, working a posted circuit announced at each arrival by the chime bar's recognized notes.

    Work is done in public at the cart, the price named before the first touch, and the repair narrated to the watching queue. A circuit mender returns twice a year for a working lifetime, so the same settlements inspect the old work at every visit. The chime bar's notes and circuit are registered with the road offices. Settlements plan repairs toward the posted season, while the cart carries news and salvaged fittings between its stops. The circuit medal on the apron says which roads a cart serves. By trade custom, everyone on those roads helps repair a disabled mender's cart.
  PROSE
end
