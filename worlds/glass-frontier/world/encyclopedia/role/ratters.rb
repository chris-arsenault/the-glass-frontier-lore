encyclopedia :ratters do
  ...
  affordance "Any Ratter in distress in the Shear is answered by every crew that can reach them. That includes crews the caller owes, crews they walked out on, and crews they were fighting the week before; the rivalry resumes afterward without discussion."
  pressure "Accepting fuel, parts, or a tow from a Ratter creates a debt repaid in kind rather than in coin, and it stays callable years later. A crew that walks one gets no flotilla, no skiff-port service, and no long-rat to speak for them, which is where #{ref :coremark, "Coremark"} finds people to hire."
  cue "Ratters read each other off the patchwork coat, the cant, and the shard pendant from a first claim, and all three are checked at once. Wearing the gear without the argot marks a player as suspect, and faking fluency in Shear-cant is caught inside a sentence."
end
