encyclopedia :far_attendant do
  ...
  prose <<~PROSE
    Every live #{encyclopedia_ref :farglass, "farglass"} pair of consequence is served at both ends, and the far attendants are the service. The office keeps the covering hours — the published schedule by which a pane's cloth lifts and lowers, held to the minute at both ends so that each room's exposure to the other is consented, bounded, and simultaneous — and witnesses everything the glass does. Each lifting enters the hour-book with its authority, witnesses, and duration; the paired books at the pane's two ends give either side a custody record it can audit against the other. Between covering hours the attendant guards the pane. During them, the attendant works the lamp codes for urgency and turn-taking, manages the slates, and records the session without joining its substance.
