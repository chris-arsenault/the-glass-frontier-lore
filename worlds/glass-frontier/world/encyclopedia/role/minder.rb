encyclopedia :minder do
  ...
  pressure "The machine outlives every explanation of it, so when it changes behavior the minder must choose between remedies that worked before and specialists whose theories contradict the log — and the whole district lives with the choice."
  ...
  prose <<~PROSE
    ...
    The log is the real instrument. A running elven machine responds to care in ways its minders can document and use — this feeding schedule holds the hum steady, that cleaning order precedes the good season, this noise arrives before that fault — and a good log holds generations of such observations in a dozen hands. Specialists arrive with theories about what the machine is; the log records what the machine does. When the two disagree, districts have learned by expensive experience to hear the log out first, and the standing of minders as a class rests on the number of times the log was right.
  PROSE

  prose <<~PROSE, section: :structure, heading: "Standing and Succession"
    A minder's authority is narrow and absolute: everything touching the machine passes the minder first, from a repair crew's tools to a festival's decorations on the housing. The great machines — settlement lungs, span counterweights — carry salaried minders with deputies and apprentices, and their logs are civic documents copied to the archive. A neighborhood water engine is minded between other work by whoever holds its log, and holding it carries standing of the durable, unglamorous kind: the minder of a working machine is somebody in any room in the district.
