encyclopedia :closer do
  ...
  pressure "The book takes everything the wake will say, and some lines close hard — the apology the recipient will refuse to hear, the debt the family calls slander, the message that should perhaps stay undelivered — and the closer's neutrality is the trade's asset and its nightly argument."
  ...
  prose <<~PROSE
    ...
    The trade's origin is the system's deepest wound worked into custom. The Famine left the accounts of the #{encyclopedia_ref :elves, "elves"} open at every scale — debts to the vanished, work promised by them, words meant for them — and the first closers were the clerks and neighbors who took on the impossible settling because unsettled accounts were unbearable at that volume. The impossible parts calcified into the trade's oldest custom: the open books of the vanished, held by houses that formally decline each year to close them, tended by retained closers whose whole duty is the annual reading of accounts that cannot be settled and will not be abandoned. Everything else the trade does descends from that discipline — the neutrality, the witnessed strikes, the even voice that delivers apology and demand alike — a profession built on the belief, tested at civilizational scale, that what the dead leave open the living can close, line by line, if someone will only walk it.
  PROSE
end
