encyclopedia :lamplighter do
  ...
  pressure "The round is long, cold, and single-handed, and every judgment on it is a route-wide fact — a lamp deferred to beat weather goes dark in the fans' rings forever, and the crews who steer by that lamp know the lighter's name."
  ...
  prose <<~PROSE
    ...
    Tending the lamp means tending its garden. #{encyclopedia_ref :cold_route_spores, "Cold-route spores"} flower wherever a lamp speaks, the fans crowd the light on their own schedule, and the lighter's fan-knife does the season's scraping — always sparing the oldest ring, by the custom that keeps the route's history legible. The scrapings ride home as trade stock, the colonies' rings audit every lighter's diligence in public and forever, and lighters speak of the fans with the mixed feelings of anyone whose work is graded by a plant: the colony is the round's burden, its pension, and its permanent record, and a lighter reading an old post's rings can tell you which predecessors deferred their winters.
  PROSE
  ...
  prose <<~PROSE, section: :structure, heading: "Rounds, Hearts, and Succession"
    ...
    Rounds pass by walking. A successor rides the full round with the incumbent at least once — learning each lamp's temper, each stretch's weather, where the book's neat lines hide hard nights — and the handover is marked at the round's last lamp, where the retiring lighter makes the final entry and the new one makes the first. The route offices countersign, the crews on the route drink to it, and the fans, in their patient way, will record how it went.
  PROSE
end
