encyclopedia :gray_sheet do
  ...
  prose <<~PROSE, section: :trade, heading: "Ghosts and What They Cost"
    ...
    Archive houses buy seasoned sheet from famous rooms at ghost prices — stock from the ring's own offices carries elven working notes in its deep layers, and raising them is a recognized scholarly craft with its own journals and its own frauds. A cottage industry of lamp-readers works the middle of the market, hired by the curious to see what the kitchen sheet remembers, and their trade custom of reading aloud only to the sheet's owner is the sort of rule that implies its own history.
  PROSE

  cue "The stall's price board is a pearly gray sheet chalk-marked in stylus, and between customers the seller wipes one line with a cloth kept warm on the kettle and writes the new price in its place."
