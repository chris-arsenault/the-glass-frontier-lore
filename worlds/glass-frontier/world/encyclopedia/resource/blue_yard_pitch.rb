encyclopedia :blue_yard_pitch do
  ...
  summary "Blue yard pitch is the working sealant of the ringglass trades: a dense blue-black paste that hardens around vibrating ringglass and peels away cleanly the moment the vibration stops. Yards pay seams with it, casters bed live glass in it, and the peel is the trade's own quality gauge."
  ...
  availability "Boiled at yard shops from mineral tar and glass fines; the famous kettles are at the Dry Net, Three Arms Yard, Blue Step Works, and Lot Forty, and yard crews argue their merits like vintages"
  ...
  prose <<~PROSE
    Ringglass under load vibrates, and everything joined to it must live with the hum. Blue yard pitch is the trades' answer: a dense blue-black paste, boiled from mineral tar and glass fines, that cures hard only against vibration. Paid warm into a running seam it sets to a glassy gasket that rides the hum like a hull rides water; on still glass it stays a dull paste indefinitely. The cure's condition is the material's whole character, and the yards use both faces — pitch seals the live assembly, and the moment the work is stopped for inspection, the rind lets go and peels away in whole ribbons, leaving the glass bare, clean, and readable.

    Paying pitch is bench craft with its own irons and its own tempo. The payer works the seam behind the kettle hand, iron in each fist, driving the warm paste into the joint while the assembly runs — the trades pay live, since the cure needs the hum — and a paid seam's rind is signed with the payer's iron marks the way a caulked hull is signed. The peel is the audit: ribbons that come away whole certify an even cure and a sound seam, and a peel that crumbles or tears sends the seam back to the payer with the ribbons as evidence. Yard fights over crumbled peel are the trade's standard drama, and the kettle shops keep peel samples the way roperies keep failed line.
  PROSE

  prose <<~PROSE, section: :trade, heading: "Kettles and Recipes"
    The four famous kettles — the Dry Net's, Three Arms Yard's, Blue Step Works', and Lot Forty's — boil to house recipes that vary in set time and peel, and yard crews defend their house pitch with vintage-cellar seriousness. Quick pitch sets in a shift and does patch work; bench pitch sets overnight for standing assemblies; and the slow deep-set that beds permanent installations cures across a week of continuous run, which is why casting shops schedule their commissioning runs unbroken and post the fact at the gate.

    The stall is the material's one betrayal, and the trades plan around it by doctrine. Pitch releases on stillness with mechanical indifference, so a sealed line that stalls unplanned is unsealing itself while the crew responds — every pitch-sealed assembly carries a stall standing order, and the pitch trades and the #{encyclopedia_ref :workshop_authority, "stop-authority"} custom lean on each other: the crew that halts a line pays the peel, and the custom prices that into every halt call, and pays it anyway.
  PROSE
  ...
  variation "The four kettle houses' recipes differ in set and peel, and a yard crew's loyalty to its house pitch survives marriages, transfers, and evidence."
end
