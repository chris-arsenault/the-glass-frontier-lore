geographic_location :dovra do
  ...
  prose <<~PROSE
    ...
    People live on natural rises, old levees, driven piles, and earth mounds raised by many rounds of communal work. Towns face water in several directions. Roads exist, but ferries, #{encyclopedia_ref :channel_barge, "Dovrans"}, and tow paths carry more local traffic. #{ref :lowbank, "Lowbank"} is the largest port and the usual transfer point between river cargo and seagoing vessels.
  PROSE
  ...
  prose <<~PROSE, section: :economy, heading: "River Work"
    ...
    #{encyclopedia_ref :channel_barge, "Dovrans"} collect produce from landing stairs and carry it to Lowbank or smaller sea ports. Most are family or village vessels with shallow hulls and removable deck houses. A crew may run the same loop for decades while changing half its course as channels move. Route knowledge belongs to pilots who read color, reed direction, bottom pressure, and the tone returned by a pole against buried iron sand. Near reed banks, a #{encyclopedia_ref :pole_thief, "tolven"} can add a convincing false strike.
