artifact :echo_strip do
  ...
  prose <<~PROSE, section: :present_day, heading: "The Answering Strips"
    #{ref :quiet_measure, "*Quiet Measure*"} carried echo strips on its isolated receiver after the repeated distress pattern appeared beyond Pell. Two strips captured the call. Both also hold the tender's acknowledgment along the same physical margin, although the ship's transmitter remained disconnected.
...
relate :rel_echo_conclave_possesses_echo_strip, :possesses, :echo_ledger_conclave, :echo_strip do
  prose "The Conclave manufactures archive-grade strips and authenticates field records from their folds and margins."
end
relate :rel_echo_strip_depends_ringglass, :depends_on, :echo_strip, :ringglass do
  prose "Powdered ringglass shifts inside the foil to retain one coherent signal pattern."
end
relate :rel_echo_strip_depends_stillwater, :depends_on, :echo_strip, :stillwater do
  prose "Stillwater resin holds the grains loose during recording and fixes them when the strip is sealed."
end
relate :rel_quiet_measure_carries_echo_strip, :carries, :quiet_measure, :echo_strip do
  prose "The rescue tender uses isolated strips to record distress calls without joining them to its signal bus."
end
