resource :pre_glassfall_transmissions do
  ...
  prose <<~PROSE, section: :description, heading: "Where They Remain"
    The richest source is the #{ref :echo_rivers, "Echo Rivers"}, which lift old signal from crystal-laced rock and carry it to surface pools. The same process occurs in smaller places: flooded service channels, old foundations, sealed conduits, and ringglass walls that absorbed years of local traffic.

    Fallen ring fragments preserve a different record. Some retain partial power and still transmit on fixed intervals. #{ref :the_choir_fragment, "The Choir Fragment"} sends the same message every week with minor changes in two words. Other fragments are silent until a #{ref :tuners, "Tuner"} supplies the missing carrier tone.
...
relate :rel_pre_glassfall_transmissions_created_ring_age, :created_during, :pre_glassfall_transmissions, :the_ring_age do
  prose "The signals were created while the orbital ring still carried the system's daily traffic."
end
relate :rel_pre_glassfall_transmissions_resonate_ringglass, :resonates_with, :pre_glassfall_transmissions, :ringglass do
  prose "#{ref :ringglass, "Ringglass"} preserves signal-band impressions in walls, shards, and surviving ring volumes."
end
