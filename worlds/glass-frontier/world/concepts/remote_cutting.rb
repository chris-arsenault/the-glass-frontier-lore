concept :remote_cutting do
  ...
  prose <<~PROSE
    Remote cutting lets a salvage crew work a claim without bringing its people alongside the target. An unmanned cutter enters the debris mass, fixes itself to a chosen seam, and couples that material to a receiving breach on the crew's #{ref :claim_rig, "claim rig"}. The crew stays far enough away to survive a collapse, a resonance spike, or the loss of the cutter.

    #{ref :ratters, "Ratter"} crews developed the method for #{ref :deep_shear, "Deep Shear"} claims that were valuable enough to work and too unstable to board. Crews plan every cut around losing the machine and bringing its operators home.
  PROSE
  prose <<~PROSE, section: :operations, heading: "A Remote Cut"
    A cutter carries an anchor head, a compact resonance array, and little else. Once it reaches the target, the crew sounds the seam in several narrow bands. A #{ref :tuners, "Tuner"} selects the response that separates the desired material from the surrounding mass, and the cutter holds that tone while the #{ref :claim_rig, "claim rig"} opens a matching receiver.
  ...
  prose <<~PROSE, section: :dangers, heading: "The Cutter Wake"
    A coherent return carries whatever is coupled to the selected seam. Dust and loose hardware are expected. Small #{ref :farborn, "Farborn"} sometimes arrive alive, clinging to the recovered material or following the return tone into the receiving breach.

    Crews call this a **cutter wake**. The outer bay closes, the inner rooms go quiet, and the crew clears the breach before processing resumes. Most #{ref :claim_rig, "claim rigs"} carry simple barriers and tuned deck weapons for this purpose. A wake becomes dangerous when an operator leaves several process rooms active: the intruders follow the strongest tone inward, and every open room gives them another route.
  PROSE
...
relate :rel_remote_cutting_depends_on_resonance, :depends_on, :remote_cutting, :resonance do
  prose "Remote cutting uses #{ref :resonance, "resonance"} to couple a chosen seam to a distant receiving breach."
end
relate :rel_remote_cutting_resonates_with_farborn, :resonates_with, :remote_cutting, :farborn do
  prose "The return tone can carry small #{ref :farborn, "Farborn"} into a #{ref :claim_rig, "claim rig"}, a hazard crews call the cutter wake."
end
