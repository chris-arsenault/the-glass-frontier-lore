...
function BrowseControls({ world, kind, tag, query, update }: BrowseControlsProps) {
  return (
    <div className="browse-controls">
      ...
      <label><span>Kind</span><select value={kind} onChange={(event) => update("kind", event.target.value)}>
...
function BrowseResults({ entries, update }: { entries: EntrySummary[]; update: BrowseControlsProps["update"] }) {
  return <div className="browse-results">{entries.map((entry) => (
    <article className="browse-row" key={entry.id}>
      <div className="browse-row__meta"><span>{entry.kind.replaceAll("_", " ")}</span>{entry.prominence && <span>{entry.prominence}</span>}</div>
  ...
  return (
    <main className="reader-page browse-view">
      <ViewHeader
        ...
        actions={null}
      />
      <BrowseControls world={world} kind={kind} tag={tag} query={query} update={update} />
