npc :lira_vashtenri do
  ...
  prose <<~PROSE, section: :public_profile, heading: "Public Profile"
    ...
    Nobody noticed until the Bloom. When the #{ref :bloom_coalition, "Bloom Coalition"} needed containment expertise and nothing was working, someone traced the maintenance records and found a refinery that hadn't reported the chronic issues plaguing every other Shear installation. They traced it to the mechanic.
  ...
  PROSE
  prose <<~PROSE, section: :history, heading: "History"
    After developing the containment buoy network, Lira founded the #{ref :displacement_council, "Displacement Council"}'s containment operations — scaling her instinctive approach into a training program. She didn't lead the Council (that was #{future "Oram Sells"}, then #{ref :dern_talish, "Dern Talish"}), but the containment methodology is hers.
