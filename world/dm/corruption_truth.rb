dm :corruption_truth do
  name "Resonance Corruption — DM Truth"
  path "dm/corruption-truth.md"
  tags :resonance, :danger
  dm! public_entry: :duskgrain

  prose <<~PROSE
    Public entry: #{ref :duskgrain, "Duskgrain"}
  PROSE
  prose <<~PROSE, section: :truth, heading: "The Corruption"
    Duskgrain is not a natural byproduct of sustained resonance. It is the physical residue of the #{ref :the_false_form_truth, "Adversary"}'s influence bleeding through reality tears.

    Where the Adversary's presence touches normal space — in Bloom Zones, in deep Shear pockets, in places where narrow-bandwidth resonance has weakened the boundary — it leaves behind crystallized fragments of itself. These are duskgrain. The substance is, in a very literal sense, solidified alien intent — or, in the language of #{ref :the_false_form_truth, "The False Form"}, solidified non-closing figure: fragments of a geometry that cannot exist, frozen into matter that nonetheless does.
  ...
  PROSE
  prose <<~PROSE, section: :truth, heading: "The Elves Know"
