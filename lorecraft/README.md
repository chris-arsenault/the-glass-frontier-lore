...
- **Kind facts:** `extend_kind :npc do ... end` declares the ordered facts that
  ...
  can set `require_fact_cards! from: :renowned, minimum: 4`; lint then rejects
  any public entry at that prominence or above whose infobox resolves fewer
  facts. `extend_subkind` adds world-specific fields to one existing subkind.
  The facts report inventories entries below the configured minimum.
